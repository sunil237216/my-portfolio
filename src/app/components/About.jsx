     ...
     <div className='flex-1'>
      ...
      <ul className='grid grid-cols-1 sm:grid-cols-3 gap-6 max-w-2xl mx-auto'>
        {infoList.map(({icon, iconDark, title,description},index) => (
          <li key={index} className='border-[0.5px] border-gray-400 rounded-xl p-6 cursor-pointer hover:bg-sky-100 hover:-translate-y-1 duration-500 flex flex-col h-full'>
            <Image src={icon} alt={title} className='w-7 mb-3' />
            <div>
              <h3 className='font-Ovo text-lg mb-2'>{title}</h3>
      ...
      <ul className='flex justify-center gap-3 sm:gap-5 mx-auto'>
